...
/**
    ...
    The frequency axis is logarithmic, from 20 Hz to the top of the audio: an
    octave takes the same height wherever it is, as it's heard, rather than
    the top octave taking half the view as it would on a linear axis. Levels
...
namespace spectrogramimage
{
    ...
    inline double frequencyAt(double proportion, double nyquist)
    {
        const double top = std::max(kLowestHz * 2.0, nyquist);
        return kLowestHz * std::pow(top / kLowestHz, std::clamp(proportion, 0.0, 1.0));
    }

    /** How far up the axis @p hz sits (0 bottom, 1 top). */
    inline double proportionOf(double hz, double nyquist)
    {
        const double top = std::max(kLowestHz * 2.0, nyquist);
    ...
    inline juce::Image imageOf(const engine::SpectrogramData& data, int rows = 256)
    {
        ...
        for (int row = 0; row < rows; ++row)
        {
            ...
            const int    binLow = std::clamp((int) std::floor(data.binOfFrequency(frequencyAt(bottom, nyquist))), 0, data.bins - 1);
            const int    binHigh = std::clamp((int) std::ceil(data.binOfFrequency(frequencyAt(top, nyquist))), binLow, data.bins - 1);
