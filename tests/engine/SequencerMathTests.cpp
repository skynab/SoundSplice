...
using Catch::Approx;
using soundsplice::engine::edgeInBlock;
using soundsplice::engine::wrapPositive;
...
TEST_CASE("A loop runs over what has been arranged", "[engine][loop]")
{
    ...
    REQUIRE(soundsplice::engine::loopEndForContent(16.0, 4.0) == 16.0); // exactly four bars
    REQUIRE(soundsplice::engine::loopEndForContent(13.0, 4.0) == 16.0); // rounded up to the bar
    REQUIRE(soundsplice::engine::loopEndForContent(0.5, 4.0)  == 4.0);
}
...
TEST_CASE("An empty song still gets a loop of one bar", "[engine][loop]")
{
    ...
    REQUIRE(soundsplice::engine::loopEndForContent(0.0, 4.0) == 4.0);
    REQUIRE(soundsplice::engine::loopEndForContent(-5.0, 4.0) == 4.0);
    REQUIRE(soundsplice::engine::loopEndForContent(0.0, 3.0) == 3.0); // and in 3/4
}

TEST_CASE("The loop end follows the time signature", "[engine][loop]")
{
    REQUIRE(soundsplice::engine::loopEndForContent(10.0, 3.0) == 12.0); // 3/4: four bars
    REQUIRE(soundsplice::engine::loopEndForContent(10.0, 5.0) == 10.0); // 5/4: two bars exactly
}

TEST_CASE("A nonsense bar length falls back rather than dividing by zero", "[engine][loop]")
{
    REQUIRE(soundsplice::engine::loopEndForContent(10.0, 0.0) == 12.0); // treated as 4/4
    REQUIRE(std::isfinite(soundsplice::engine::loopEndForContent(10.0, -1.0)));
}
...
TEST_CASE("Play at the end of the arrangement starts it again", "[engine][transport]")
{
    ...
    REQUIRE(soundsplice::engine::shouldRestartFromStart(16.0, 16.0));
    REQUIRE(soundsplice::engine::shouldRestartFromStart(20.0, 16.0)); // stopped a little past it
}

TEST_CASE("Play from the middle carries on from there", "[engine][transport]")
{
    REQUIRE_FALSE(soundsplice::engine::shouldRestartFromStart(0.0, 16.0));
    REQUIRE_FALSE(soundsplice::engine::shouldRestartFromStart(8.0, 16.0));
    REQUIRE_FALSE(soundsplice::engine::shouldRestartFromStart(15.0, 16.0));
}
...
TEST_CASE("Landing a hair short of the end still counts as the end", "[engine][transport]")
{
    ...
    REQUIRE(soundsplice::engine::shouldRestartFromStart(16.0 - 1.0e-9, 16.0));
    REQUIRE(soundsplice::engine::shouldRestartFromStart(16.0 - 1.0e-4, 16.0));
    ...
    REQUIRE_FALSE(soundsplice::engine::shouldRestartFromStart(15.99, 16.0));
}

TEST_CASE("With nothing arranged there is no end to restart from", "[engine][transport]")
{
    REQUIRE_FALSE(soundsplice::engine::shouldRestartFromStart(0.0, 0.0));
    REQUIRE_FALSE(soundsplice::engine::shouldRestartFromStart(50.0, 0.0));
}
...
TEST_CASE("Seconds convert to beats at the song's tempo", "[engine][tempo]")
{
    ...
    REQUIRE(soundsplice::engine::beatsForSeconds(2.0, 120.0) == Approx(4.0));
    REQUIRE(soundsplice::engine::beatsForSeconds(2.0, 60.0) == Approx(2.0));
    REQUIRE(soundsplice::engine::beatsForSeconds(30.0, 120.0) == Approx(60.0));
}
...
TEST_CASE("A long recording is a long clip", "[engine][tempo]")
{
    ...
    const double beats = soundsplice::engine::beatsForSeconds(30.0, 120.0);
    REQUIRE(beats > 4.0);
...
TEST_CASE("A nonsense duration or tempo converts to nothing", "[engine][tempo]")
{
    ...
    REQUIRE(soundsplice::engine::beatsForSeconds(0.0, 120.0) == 0.0);
    REQUIRE(soundsplice::engine::beatsForSeconds(-5.0, 120.0) == 0.0);
    REQUIRE(soundsplice::engine::beatsForSeconds(10.0, 0.0) == 0.0);
    REQUIRE(soundsplice::engine::beatsForSeconds(10.0, -120.0) == 0.0);
}
...
TEST_CASE("A beat division converts to Hz at the song's tempo", "[engine][tempo]")
{
    ...
    REQUIRE(soundsplice::engine::hzForBeatDivision(120.0, 1.0) == Approx(2.0));
    REQUIRE(soundsplice::engine::hzForBeatDivision(120.0, 0.5) == Approx(4.0));
    REQUIRE(soundsplice::engine::hzForBeatDivision(240.0, 1.0) == Approx(4.0));
}
...
TEST_CASE("A nonsense tempo or division converts to no rate", "[engine][tempo]")
{
    ...
    REQUIRE(soundsplice::engine::hzForBeatDivision(0.0, 1.0) == 0.0);
    REQUIRE(soundsplice::engine::hzForBeatDivision(-120.0, 1.0) == 0.0);
    REQUIRE(soundsplice::engine::hzForBeatDivision(120.0, 0.0) == 0.0);
    REQUIRE(soundsplice::engine::hzForBeatDivision(120.0, -1.0) == 0.0);
}
